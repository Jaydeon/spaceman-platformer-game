...
class Design_5_5_SlimeBehavior extends ActorScript
{
	...
	override public function init()
	{
		...
		addWhenUpdatedListener(null, function(elapsedTime:Float, list:Array<Dynamic>):Void
		{
			if(wrapper.enabled)
			{
				...
				if(!(_AttackDelay))
				{
					if(((Math.abs((_PlayerActor.getXCenter() - actor.getXCenter())) <= _FireRange) && ((_PlayerActor.getY() + (_PlayerActor.getHeight())) == (actor.getY() + (_PlayerActor.getHeight())))))
					{
						createRecycledActor(_Arrow, actor.getXCenter(), actor.getYCenter(), Script.BACK);
						...
						runLater(1000 * (actor.getNumFrames() * .5), function(timeTask:TimedTask):Void
						{
